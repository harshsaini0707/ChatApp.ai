  ...
  const fetchMessages = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}messages/${user.id}/${selected.id}`,
  ...
  return (
    <div className="relative text-white h-screen w-screen flex font-sans">
      ...
      <div className="relative z-10 flex w-full h-full">
        ...
        {(!isMobile || !selected) && (
          <div className="flex flex-col w-full md:w-1/4 min-w-[280px] max-w-[320px] bg-black/40 backdrop-blur-xl border-r border-slate-300/10">
            {/* User info */}
            <div className="flex items-center justify-between p-4 border-b border-slate-300/10">
              ...
              <button
                onClick={logOut}
                className="px-3 py-1.5 hover:bg-red-600/80 text-white rounded-lg text-sm font-semibold transition-colors duration-200"
              >
                <LogOut />
              </button>
            </div>
    ...
    scale: 1
                      ...
                      >
                        <div
                          className={`inline-block px-4 py-2 rounded-t-xl shadow-md max-w-[70%] break-words ${
                            mine
